...
defmodule BotdWeb.CoreComponents do
  ...
  def header(assigns) do
    ...
    <header class={[@actions != [] && "flex items-center justify-between", @class]}>
      ...
      <div class="w-96 grid grid-cols-4 m-2">{render_slot(@actions)}</div>
    </header>
    """
  end
  ...
  @doc """
  Renders a table with colored header and compact cells.

  ## Examples

      <.table rows={@suggestions}>
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(BotdWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(BotdWeb.Gettext, "errors", msg, opts)
    end
  end
